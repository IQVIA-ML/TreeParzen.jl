...
log_samples = GMM.GMM1(components, 10_001)
pos_samples = sort(exp.(log_samples))
edges = pos_samples[1:500:end]
pdf = exp.(LogGMM.LGMM1_lpdf(col(edges[1:end - 1]), components))
...
high = 3.5
q = 0.1
bounded_q_samples = vec(LogGMM.LGMM1(components, low, high, q, 10_001))
...
prob = vec(exp.(LogGMM.LGMM1_lpdf(col(vals), components, low, high, q)))
err = (prob .- y) .^ 2
@test maximum(err) < 0.1
@test mean(err) < 0.01
@test median(err) < 0.01

@test_throws ArgumentError LogGMM.LGMM1(components, 3.5, 3.5, 10_001)
