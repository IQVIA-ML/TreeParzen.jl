...
function posterior(
    ...
    b_post, b_components = Samplers.lognormal(
        obs_below, mu, sigma, config.draws, config
    )
    _, a_components = Samplers.lognormal(
    ...
    below_llik = LogGMM.LGMM1_lpdf(b_post, b_components)
    above_llik = LogGMM.LGMM1_lpdf(b_post, a_components)
...
function posterior(
    ...
    b_post, b_components = Samplers.logquantnormal(
        Float64.(obs_below), mu, sigma, q, config.draws, config
    )
    _, a_components = Samplers.logquantnormal(
    ...
    below_llik = LogGMM.LGMM1_lpdf(b_post, b_components, q)
    above_llik = LogGMM.LGMM1_lpdf(b_post, a_components, q)
...
function posterior(
    ...
    b_post, b_components = Samplers.normal(
        Float64.(obs_below), mu, sigma, config.draws, config
    )
    _, a_components = Samplers.normal(
    ...
    below_llik = GMM.GMM1_lpdf(b_post, b_components)
    above_llik = GMM.GMM1_lpdf(b_post, a_components)
...
function posterior(
    ...
    b_post, b_components = Samplers.quantnormal(
        Float64.(obs_below), mu, sigma, q, config.draws, config
    )
    _, a_components = Samplers.quantnormal(
        Float64.(obs_above), mu, sigma, q, config.draws, config
    )
    ...
    below_llik = GMM.GMM1_lpdf(b_post, b_components, q)
    above_llik = GMM.GMM1_lpdf(b_post, a_components, q)
...
function posterior(
    ...
    b_post, b_components = Samplers.uniform(
        obs_below, low, high, config.draws, config
    )
    _, a_components = Samplers.uniform(
    ...
    below_llik = GMM.GMM1_lpdf(b_post, b_components, low, high)
    above_llik = GMM.GMM1_lpdf(b_post, a_components, low, high)
...
function posterior(
    ...
    b_post, b_components = Samplers.quantuniform(
        Float64.(obs_below), low, high, q, config.draws, config
    )
    _, a_components = Samplers.quantuniform(
    ...
    below_llik = GMM.GMM1_lpdf(b_post, b_components, low, high, q)
    above_llik = GMM.GMM1_lpdf(b_post, a_components, low, high, q)
...
function posterior(
    ...
    b_post, b_components = Samplers.loguniform(
        float.(obs_below), low, high, config.draws, config
    )
    _, a_components = Samplers.loguniform(
...
function posterior(
    ...
    b_post, b_components = Samplers.logquantuniform(
        float.(obs_below), low, high, q, config.draws, config
    )
    _, a_components = Samplers.logquantuniform(
    ...
    below_llik = LogGMM.LGMM1_lpdf(b_post, b_components, low, high, q)
    above_llik = LogGMM.LGMM1_lpdf(b_post, a_components, low, high, q)
